...
/**
 * 季度排行榜数据
 * 每个季度的第一天的零点执行，统计上一个季度的数据: 0 0 01 *\/3 *  php /data/web/dss_crm_prod/scripts/historicalPlayRankingStatistics.php
 * 上线当天凌晨手动执行历史数据的汇总(计划2020-08-24 00:00:00 之前的数据 只执行一次）：php historicalPlayRankingStatistics.php -s=0 -e=1598198400
 *
 */
...
try {
    //当前年月日
    $time = time();
    //支持命令行传入 或 自动获取演奏记录开始和结束时间戳的范围
    $cliParams = getopt('s:e:');
    $studentIdJoinRanking = Constants::STATUS_TRUE;
    if (isset($cliParams['s']) && isset($cliParams['e'])) {
        $startTime = (int)$cliParams['s'];
        $endTime = (int)$cliParams['e'];
        $quarterNum = date("Y", $endTime) . '2';
        $studentIdJoinRanking = Constants::STATUS_FALSE;
    } else {
        // 获取上一个季度演奏记录统计的起始时间
