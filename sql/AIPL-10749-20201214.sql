...
('CHECKIN_PUSH_CONFIG', '打卡签到设置', 'day_2', '{\"content1\":\"\\\\ud83d\\\\udcaa已经坚持练琴2天啦，音准和节奏一定进步了不少，好棒哦！\\n\\n\\\\ud83d\\\\udd25爱练琴爱分享，打卡返学费活动第2⃣️天！\\n\\n\\\\ud83c\\\\udfaf打卡方式：复制以下【文案➕海报】分享至朋友圈，即可领取打卡红包[红包]\\n[爱心]活动详情【<a href=\\\"{page_url}\\\">活动入口链接</a>】\",\"content2\":  \"宝贝能爱上钢琴，真是一件非常幸运的事情[哇]～ 宝贝今天继续用“小叶子智能陪练”练琴，智能纠错，及时反馈，宝贝的错音越来越少！[加油！][加油！]\"}', '第2天推送配置'),
...
SET @task_id = (SELECT id FROM dss_pre.erp_event_task WHERE `type`=13 AND `name`='打卡1天' LIMIT 1); 
...
SET @task_id = (SELECT id FROM dss_pre.erp_event_task WHERE `type`=13 AND `name`='打卡3天' LIMIT 1); 
...
SET @task_id = (SELECT id FROM dss_pre.erp_event_task WHERE `type`=13 AND `name`='打卡5天' LIMIT 1); 
INSERT INTO `dict`(`type`, `key_name`, `key_code`, `key_value`, `desc`) VALUES ('operation_common_node', '通用红包搜索节点', '22', '分享返学费打卡5天 - 9.9元', '');
...
CREATE TABLE `share_poster` (
  ...
  `type` tinyint(1) unsigned DEFAULT '1' COMMENT '海报类型：1打卡上传截图',
  `verify_status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '审核状态 1待审核 2合格 3不合格',
  `verify_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '审核时间',
  `verify_user` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '审核人uid',
  `verify_reason` varchar(255) NOT NULL DEFAULT '' COMMENT '审核原因',
