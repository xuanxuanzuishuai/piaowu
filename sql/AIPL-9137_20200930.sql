CREATE TABLE `student_sign_up` (
  ...
  `start_time` int(11) NOT NULL COMMENT '开课时间（小时:分）',
  `last_course_time` int(11) NOT NULL COMMENT '最后一节课上课时间',
  `bind_status` tinyint(4) NOT NULL DEFAULT '1' COMMENT '报名状态，1：报名成功； 2：取消报名',
  `create_time` int(11) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `update_time` int(11) NOT NULL DEFAULT '0' COMMENT '更新时间',
  `first_course_time` int(11) NOT NULL DEFAULT '0' COMMENT '第一节课上课时间',
  PRIMARY KEY (`id`)
) COMMENT='用户报名表';
...
CREATE TABLE `student_learn_record` (
  ...
  `sort` int(11) NOT NULL COMMENT '课程顺序',
