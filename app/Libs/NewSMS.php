...
※请立即添加助教老师微信：{$wechatcs}，并发送购课手机号。";
    ...
    public function sendCollectionCompleteNotify($targetMobile, $sign, $collectionList)
    {
        ...
        $msg = "恭喜您成功购买小叶子智能陪练，开营前邀请您试用智能陪练！请您尽快关注微信公众号【小叶子智能陪练】绑定账号获得APP下载链接，我们的课程将于{$teachingStartDate}（{$week}）开始，时长{$days}天，请您务必查看开营指引【".$_ENV['SMS_FOR_EXPERIENCE_CLASS_REGISTRATION']."?class_id=".$collectionList['id']."】。如有疑问，请拨打客服电话：".$_ENV['AI_SERVER_TEL']."。";
        $data = [
            'sign_name' => $sign,
            'phone_number' => $targetMobile,
            'content' => $msg,
