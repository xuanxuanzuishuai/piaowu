...
class ActivityLottery extends ControllerBase
{
	...
	public function addOrUpdate(Request $request, Response $response): Response
	{
		$rules = [
			...
			[
				'key'        => 'win_prize_rule',
				'type'       => 'required',
				'error_code' => 'win_prize_rule_is_required'
			],
			[
				'key'        => 'enable_status',
				'type'       => 'in',
			...
			[
				'key'        => 'material_send_interval_hours',
				'type'       => 'required',
				'error_code' => 'material_send_interval_hours_is_required'
			],
			[
				'key'        => 'material_send_interval_hours',
				'type'       => 'min',
				'value'      => 1,
				'error_code' => 'material_send_interval_hours_min_value_error'
			],
			[
				'key'        => 'upper_limit',
				'type'       => 'max',
				'value'      => 100,
				'error_code' => 'upper_limit_max_value_error'
			],
			[
				'key'        => 'upper_limit',
				'type'       => 'min',
				'value'      => -1,
				'error_code' => 'upper_limit_min_value_error'
			],
		];
	...
	public function appendImportUser(Request $request, Response $response): Response
	{
		...
		$params['is_cover'] = $params['is_cover'] ?? Constants::STATUS_FALSE;
		try {
			LotteryAdminService::appendImportUserData($params['op_activity_id'], $this->ci['employee']['uuid'],
				$params['is_cover']);
		} catch (RunTimeException $e) {
			return HttpHelper::buildErrorResponse($response, $e->getWebErrorData());
		}
	...
	public function updateShippingAddress(Request $request, Response $response): Response
	{
		$rules = [
			...
			[
				...
				'type'       => 'min',
				'value'      => 1,
				'error_code' => 'record_id_is_integer'
			],
			[
				'key'        => 'address_id',
				'type'       => 'required',
			...
			[
				...
				'type'       => 'min',
				'value'      => 1,
				'error_code' => 'address_id_is_integer'
			],
		];
		$params = $request->getParams();
		$result = Valid::appValidate($params, $rules);
		...
		try {
			LotteryAdminService::updateShippingAddress($params);
		} catch (RunTimeException $e) {
			return HttpHelper::buildErrorResponse($response, $e->getWebErrorData());
		}
		return HttpHelper::buildResponse($response, []);
	...
	public function cancelDeliver(Request $request, Response $response): Response
	{
		...
		try {
			LotteryAdminService::cancelDeliver($params, $this->ci['employee']['uuid']);
		} catch (RunTimeException $e) {
			return HttpHelper::buildErrorResponse($response, $e->getWebErrorData());
		}
		return HttpHelper::buildResponse($response, []);
	...
	public function updateEnableStatus(Request $request, Response $response): Response
	{
		...
		try {
			LotteryAdminService::updateEnableStatus($params['op_activity_id'], $params['status'],
	 ...
	 * @return Response
	 */
	public function activityDefaultImgConfig(
		/** @noinspection PhpUnusedParameterInspection */ Request $request,
		Response $response
	): Response {
		$dictData = DictConstants::getTypesMap([DictConstants::LOTTERY_CONFIG['type'], DictConstants::AWARD_IMG_CONFIG['type']]);
		$data = [
			'activity' => [
				'default_top_banner' => AliOSS::replaceCdnDomainForDss($dictData[DictConstants::LOTTERY_CONFIG['type']]['default_top_banner']['value']),
			],
			'award'    => (object)AliOSS::batchReplaceCdnDomainForDss($dictData[DictConstants::AWARD_IMG_CONFIG['type']])
		];
		return $response->withJson([
			'code' => Valid::CODE_SUCCESS,
			'data' => $data
