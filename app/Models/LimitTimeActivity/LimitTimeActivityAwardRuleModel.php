...
class LimitTimeActivityAwardRuleModel extends Model
{
	...
	public static function getActivityAwardRule($activityId, $key = '')
	{
		...
		return empty($key) ? $list : array_column($list, null, $key);
	}
	 ...
	 * @return int
	 */
	public static function getActivityAwardMaxTaskNum(int $activityId): int
	{
		$db = MysqlDB::getDB();
		return $db->max(self::$table, 'task_num', ['activity_id' => $activityId]);
	}
}
