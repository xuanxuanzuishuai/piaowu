...
from ba_apply a left join shop_belong_manage s on a.shop_id = s.shop_id left join shop_info p on a.id = p.id';
    ...
    {
        $where = self::buildWhere($params);
        $countSql = 'select count(*) count_num from ba_apply a left join shop_info s on s.shop_number = a.shop_number
...
from ba_apply a left join shop_info s on s.shop_number = a.shop_number
