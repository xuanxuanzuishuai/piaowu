...
class MiniAppQrService
{
    ...
    public static function getAgentQRAliOss($userId, $extParams = [])
    {
        ...
        //检测二维码是否已存在
        $res = ParamMapModel:: getQrUrl(
            $userId, $appId, $type, $paramInfo['c'], $paramInfo['a'],
            $paramInfo['e'], $paramInfo['p'], $paramInfo['lt']);
        if (!empty($res['qr_url'])) {
            return $res;
        }
        //生成小程序码
        try {
            ...
            [$imagePath, $paramsId] = PosterService::getMiniappQrImage($appId, $params, UserWeiXinModel::BUSI_TYPE_AGENT_MINI);
            if (empty($imagePath)) {
                return [];
            }
            //记录二维码图片地址数据
            ParamMapModel::updateParamInfoQrUrl($paramsId, $imagePath);
        } catch (\Exception $e) {
            SimpleLogger::error('make agent qr image exception', [print_r($e->getMessage(), true)]);
            return [];
        }
        return [$userQrTicket, $imagePath, $paramsId];
    }
}