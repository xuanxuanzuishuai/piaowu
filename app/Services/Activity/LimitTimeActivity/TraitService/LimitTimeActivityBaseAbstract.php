...
abstract class LimitTimeActivityBaseAbstract
{
     ...
     * @return array
     */
    public static function getSharePosterList($shaPosterList)
    {
        $returnData = [];
        if (empty($shaPosterList)) return $returnData;
        ...
        $templatePosterList = TemplatePosterModel::getRecords(['id' => array_merge($posterIds, $personalityPosterIds)], $field);
    ...
    public static function formatActivityInfo($activityInfo)
    {
        ...
        $info['format_target_user_first_pay_time_start'] = !empty($activityInfo['target_user_first_pay_time_start']) ? date("Y-m-d H:i:s", $activityInfo['target_user_first_pay_time_start']) : '';
        $info['format_target_user_first_pay_time_end'] = !empty($activityInfo['target_user_first_pay_time_end']) ? date("Y-m-d H:i:s", $activityInfo['target_user_first_pay_time_end']) : '';
    ...
    public static function formatActivityTimeStatus($activityInfo, $time = 0)
    {
        ...
        $activityInfo['activity_time_status_zh'] = DictService::getKeyValue('activity_time_status', $activityInfo['activity_time_status']);
        return $activityInfo;
    }
