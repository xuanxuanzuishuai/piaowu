...
class LotteryAdminService
{
	...
	public static function addOrUpdate($paramsData): int
	{
		...
		if (isset($paramsData['op_activity_id'])) {
			$checkRes = self::checkStopUpdateColumn($paramsData['op_activity_id'], $formatParams);
			$res = LotteryActivityService::update($paramsData['op_activity_id'], $checkRes);
		} else {
	...
	private static function formatParams($paramsData): array
	{
		...
		$formatParams['base_data']['material_send_interval_hours'] = $paramsData['material_send_interval_hours'];
		$formatParams['base_data']['upper_limit'] = $paramsData['upper_limit'] ?? -1;
		//参与用户规则
	...
	/**
	 * 检测不可以修改的数据:中奖规则所有信息全部可以修改，直接使用新数据，不再处理
	 * @param int $opActivityId
	 * @param array $activityParamsData
	 * @return array[]
	 * @throws RunTimeException
	 */
	private static function checkStopUpdateColumn(int $opActivityId, array $activityParamsData): array
	{
		...
		if (empty($mysqlBaseData)) {
			...
			//基础信息
			foreach ($activityParamsData['base_data'] as $bk => $bv) {
				if (!in_array($bk, $allowUpdateColumn['base_data'])) {
					unset($activityParamsData['base_data'][$bk]);
					continue;
				}
	...
	{
		$importData = ExcelImportFormat::formatImportExcelData(true, ['uuid', 'rest_times', 'order_amount']);
		foreach ($importData as $k => &$v) {
			if (strlen($v['uuid']) != 18 ||
				(int)$v['rest_times'] <= 0 ||
				(int)$v['order_amount'] < 0) {
				unset($importData[$k]);
			}
			$v['order_amount'] = (int)$v['order_amount'];
		}
	...
	public static function appendImportUserData($opActivityId, $employeeUuid, $isCover): bool
	{
		...
		foreach ($importData as &$iv) {
			...
			$iv['order_amount'] = $iv['order_amount'] * 100;
		}
	...
	private static function formatListData($list): array
	{
		...
		$dictData = DictConstants::getTypesMap([
			...
			DictConstants::PACKAGE_APP_NAME['type'],
		]);
		//操作人信息
	...
	{
		//避免多次推送erp，加锁处理
		$lockRes = Util::setLock(LotteryAwardRecordModel::LOTTERY_ENTITY_AWARD_PUSH_ERP_LOCK, 5);
		if ($lockRes === false) {
			throw new RuntimeException(["service_busy_try_later"]);
		}
		$updateRes = LotteryAwardRecordService::updateAwardShippingAddress($params['record_id'], $params['address_id']);
		if (empty($updateRes)) {
			throw new RuntimeException(["update_failure"]);
		}
		return true;
