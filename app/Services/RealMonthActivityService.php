...
class RealMonthActivityService
{
    /**
     * 添加周周领奖活动
     * @param $data
    ...
    public static function add($data, $employeeId)
    {
        ...
        $monthActivityId = RealMonthActivityModel::insertRecord($monthActivityData);
    ...
    public static function searchList($params, $page, $limit)
    {
        ...
        list($list, $total) = RealMonthActivityModel::searchList($params, $limitOffset);
    ...
    {
        $activityInfo = RealMonthActivityModel::getDetailByActivityId($activityId);
    ...
    public static function editEnableStatus($activityId, $enableStatus, $employeeId)
    {
        ...
        $activityInfo = RealMonthActivityModel::getRecord(['activity_id' => $activityId]);
        if (empty($activityInfo)) {
            throw new RunTimeException(['record_not_found']);
        }
        ...
        if ($enableStatus == OperationActivityModel::ENABLE_STATUS_ON) {
            $startActivity = RealMonthActivityModel::checkTimeConflict($activityInfo['start_time'], $activityInfo['end_time']);
        ...
        // 修改启用状态
        $res = RealMonthActivityModel::updateRecord($activityInfo['id'], ['enable_status' => $enableStatus, 'operator_id' => $employeeId, 'update_time' => time()]);
        if (is_null($res)) {
    ...
    public static function edit($data, $employeeId)
    {
        ...
        $monthActivityInfo = RealMonthActivityModel::getRecord(['activity_id' => $activityId]);
