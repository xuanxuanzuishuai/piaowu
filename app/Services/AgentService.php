...
//        }
    ...
    public static function getAgentRecommendDuplicationBills($params)
    {
        ...
        //查询订单的详细数据
        $giftCodeDetail = array_column((new Dss())->getBillDetail(array_column($billList['list'], 'parent_bill_id')), null, 'parent_bill_id');
        //查询成单代理商数据&查询归属代理商数据
    ...
    private static function formatAgentData($agentData, $agentEmployeeIds = [] ,$esIds = [])
    {
        $employeeName = [];
        if (!empty($esIds)) {
            $employeeName = array_column(EmployeeModel::getRecords(['id' => $esIds], ['id', 'name']) ?? [], 'name', 'id');
        }
        $province = $city = [];
    ...
    public static function getAgentOrderList($agentId, array $params, $page, $limit)
    {
        ...
        $giftCodeArr = [];
        if (!empty($orderIdArr)) {
            $giftCodeArr = (new Dss())->getBillDetail($orderIdArr);
    ...
    public static function recommendBillsList($params, $employeeId)
    {
        ...
        //查询订单的详细数据
        $giftCodeDetail = array_column((new Dss())->getBillDetail(array_column($recommendBillsList['list'], 'parent_bill_id')), null, 'parent_bill_id');
